...
class AdminAmenities extends CI_Model {
	...
	var $table = 'amenities';
  	var $select_fields = 'id,amenities_name,create_date,update_date,status,industry_id';
   	var $where_condition = "id!='0'";
    var $column_order = array('id','id','industry_name','create_date','update_date','status','industry_id',null); //set column field database for datatable orderable
    var $column_search = array('id','industry_name','create_date','update_date','status','industry_id'); //set column field database for datatable searchable just firstname , lastname , address are searchable
	...
	{
		$this->db->select('id,industry_id,amenities_name,status');
		$this->db->from($this->table);
		$this->db->where('id',$id);
		$query = $this->db->get();		
		return $query->row();
